...
> v2.0 — ランキング（F-07）・就活警察（F-08）削除、タブ構成変更、各種イベントタブ新設
...
**役割：** 認証ステータス・通知設定・プライバシー設定・脱退・追放を管理。プロフィール画面の⚙️ボタンから遷移。
...
SurvivorCounter       -- 生存者数リアルタイム表示
AuthStatusBadge       -- 🎓正規メンバー / 🕵️怪しいやつ バッジ
